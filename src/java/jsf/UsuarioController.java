...
public class UsuarioController implements Serializable {
    ...
    public Usuario getSelected() {
        if (current == null) {
            current = new Usuario();
            current.setUsuarioPK(new jpa.entities.UsuarioPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class UsuarioControllerConverter implements Converter {
        ...
        jpa.entities.UsuarioPK getKey(String value) {
            jpa.entities.UsuarioPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new jpa.entities.UsuarioPK();
            key.setUsuCodigo(values[0]);
            key.setPerCodigo(values[1]);
            return key;
        }

        String getStringKey(jpa.entities.UsuarioPK value) {
            StringBuffer sb = new StringBuffer();
            sb.append(value.getUsuCodigo());
            sb.append(SEPARATOR);
